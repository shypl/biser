...
public class Parser {
	...
	private void parseApiGate(TokenStream stream) throws ParserException {
		ApiGate gate = modelBuilder.getApiGate(stream.next(Token.WORD).getValue());
		TokenEntry entry = stream.next();
	...
	private void parseApiService(TokenStream stream, ApiGate gate, String name) throws ParserException {
		...
		while (true) {
			...
			try {
				...
				throw new ParserException("Error on parse api service " + gate.getName() + "." + name, e);
			}
		}
	}

	private ApiAction parseApiAction(TokenStream stream, String name, ApiSide side, boolean global, String namePrefix) throws ParserException {
