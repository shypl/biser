...
public class JavaCodeBuilder extends OopCodeBuilder {
	...
	public void buildEntity(EntityType type) {
		...
		method = cls.addMethod("getBiserEntityClassId");
...
//					.addStatement(statementIf)
		...
		private CodeClass buildServerService(CodeClass gate, ApiService service, CodeStatementSwitchCase serviceCase) {
			...
			for (ApiServiceMethod action : service.getServerActions()) {
				...
				if (action.hasArguments() || action.hasResult()) {
				...
				CodeExpressionVar meter = new CodeExpressionVar("meter", engine.getClass("org.shypl.biser.csi.server.ServerCallMeter"));
				
				CodeExpression meterComplete = new CodeExpressionMethod(meter.getVariable(), "complete");
				
				executeBody.addStatement(meter.assign(
					new CodeExpressionMethod("registerServerCall",
						new CodeExpressionString(service.getName()),
						new CodeExpressionString(action.getName()))
					)
				);
				...
				if (action.hasResult()) {
					if (action.isResultDeferred()) {
						...
						executeBody.addStatement(meterComplete);
					}
					else {
