...
public class Tokenizer
{
	...
	{
		return new TokenizerException("Unexpected token " + current.token + " (" + current.line + ":" + current.pos + ")");
	}

	public void prev() throws TokenizerException
	{
		if (next == 0) {
			throw new TokenizerException("Beginning of the stream has been reached");
		}
